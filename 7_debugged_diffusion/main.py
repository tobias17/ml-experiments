...
class FusedTransformer:
   def __init__(self, vocab_size:int, ctx_pos_size:int, den_pos_size:int, n_layers:int, ctx_dim:int, den_dim:int, den_latent:int, den_timepos:int, ctx_heads:int, den_heads:int, ctx_ff_mult:int, den_ff_mult:int):
      ...
      self.latent_dim = den_latent
      self.den_tok_embed  = Embedding(den_pos_size, den_latent)
      self.den_time_embed = Embedding(den_pos_size, den_timepos)
      self.den_pos_embed  = Embedding(den_pos_size, den_timepos)
      ...
      self.den_class_head = Linear(den_latent, vocab_size)
   ...
   def __call__(self, den_latent:Tensor, ctx_toks:Tensor, timesteps:Tensor, attn_mask:Tensor, detach_ctx:bool=False) -> Tensor:
      ...
      den_latent = den_latent.reshape((B,T,DS,DD))
